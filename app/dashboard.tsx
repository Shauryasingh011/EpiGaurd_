...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header */}
      <div className="dashboard-hero rounded-[32px] p-8 flex items-start justify-between">
        <div>
        <h1 className="text-3xl font-bold text-[var(--butter)] flex items-center gap-2">
          <Activity className="w-8 h-8" />
          <span suppressHydrationWarning>{t("dashboard.title")}</span>
        </h1>
        <p className="mt-2 text-[color:rgba(253,240,213,0.78)] text-wrap-balance" suppressHydrationWarning>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 rounded-2xl border border-white/15 bg-white/8 text-[var(--butter)] hover:bg-white/12 transition-colors flex items-center gap-2"
      ...
      <Alert className="hero-panel rounded-[28px]">
        <ShieldAlert className="h-4 w-4 text-blue-400" />
        <AlertDescription className="text-[var(--butter)]" suppressHydrationWarning>
      ...
      <Tabs defaultValue="live-map" className="space-y-4">
        <TabsList className="bg-card/85 border border-border rounded-2xl p-1">
              ...
              <div className="space-y-4">
                {riskData.map((place) => {
                  ...
                  return (
                    <div
                      key={place.location}
                      className={`dashboard-list-card p-4 rounded-[24px] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative transition-shadow duration-200 ${isFavorite ? 'border-2 border-blue-700 bg-gradient-to-br from-[#1c2c64] via-[#2d4fb1] to-[#5b6ff5] text-[var(--butter)] shadow-[0_12px_30px_rgba(28,44,100,0.28)]' : ''}`}
                      role={getRole("risk-meter")}
                      aria-label={getAriaLabel("risk-meter", place.location)}
                    >
                      {/* Favorite badge */}
                      {isFavorite && (
                        <span className="absolute top-2 right-2 flex items-center gap-1 bg-[linear-gradient(135deg,#1c2c64,#4f46e5)] text-[var(--butter)] text-xs font-semibold px-2 py-0.5 rounded-full shadow border border-white/10 z-10">
                    ...
                    <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <div className="h-2 w-full bg-[rgba(28,44,100,0.12)] rounded-full overflow-hidden">
      ...
      {/* Footer */}
      <div className="dashboard-band rounded-[24px] p-6 text-center text-muted-foreground text-sm">
